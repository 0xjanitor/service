#!/usr/bin/env bash
set -euf -o pipefail

# note, we use usr env as we are unsetting the current user

echo "=======UNSETTING_CI_ENV======="
sleep 1
export APPVEYOR_ACCOUNT_NAME=""
export APPVEYOR_BUILD_VERSION=""
export APPVEYOR_JOB_ID=""
export APPVEYOR_PROJECT_SLUG=""
export APPVEYOR_PULL_REQUEST_NUMBER=""
export APPVEYOR_REPO_BRANCH=""
export APPVEYOR_REPO_COMMIT=""
export APPVEYOR_REPO_NAME=""
export APPVEYOR=""
export BITRISE_APP_SLUG=""
export BITRISE_BUILD_NUMBER=""
export BITRISE_BUILD_URL=""
export BITRISE_GIT_BRANCH=""
export BITRISE_GIT_COMMIT=""
export BITRISE_IO=""
export BITRISE_PULL_REQUEST=""
export BRANCH_NAME=""
export BRANCH=""
export BUILD_ID=""
export BUILD_NUMBER=""
export BUILD_NUMBER=""
export BUILD_URL=""
export BUILDKITE_BRANCH=""
export BUILDKITE_BUILD_NUMBER=""
export BUILDKITE_BUILD_URL=""
export BUILDKITE_COMMIT=""
export BUILDKITE_JOB_ID=""
export BUILDKITE_PROJECT_SLUG=""
export BUILDKITE=""
export CHANGE_ID=""
export CI_BRANCH=""
export CI_BUILD_ID=""
export CI_BUILD_NUMBER=""
export CI_BUILD_REF_NAME=""
export CI_BUILD_REF=""
export CI_BUILD_REPO=""
export CI_BUILD_URL=""
export CI_COMMIT_ID=""
export CI_COMMIT=""
export CI_NAME=""
export CI_PROJECT_DIR=""
export CI_SERVER_NAME=""
export CI_SERVER_NAME=""
export CI=""
export CIRCLE_BUILD_NUM=""
export CIRCLECI=""
export CODECOV_ENV=""
export CODECOV_SLUG=""
export CODECOV_TOKEN=""
export CODECOV_URL=""
export COMMIT=""
export DRONE_BRANCH=""
export DRONE_BUILD_LINK=""
export ghprbActualCommit=""
export ghprbPullId=""
export ghprbSourceBranch=""
export GIT_BRANCH=""
export GIT_COMMIT=""
export GITLAB_CI=""
export JENKINS_URL=""
export MAGNUM=""
export PULL_REQUEST=""
export REPO_FULL_NAME=""
export REVISION=""

# https://docs.semaphoreci.com/ci-cd-environment/environment-variables/#semaphore-related
export SEMAPHORE=""
export SEMAPHORE_PROJECT_NAME=""
export SEMAPHORE_PROJECT_ID=""
export SEMAPHORE_JOB_NAME=""
export SEMAPHORE_JOB_ID=""
export SEMAPHORE_WORKFLOW_NUMBER=""
export SEMAPHORE_GIT_SHA=""
export SEMAPHORE_GIT_BRANCH=""
export SEMAPHORE_GIT_REPO_SLUG=""
export SEMAPHORE_GIT_PR_NUMBER=""

export SHIPPABLE=""

export TDDIUM_CURRENT_BRANCH=""
export TDDIUM_CURRENT_COMMIT=""
export TDDIUM_PR_ID=""
export TDDIUM_REPO_ROOT=""
export TDDIUM_TID=""
export TDDIUM=""
export TRAVIS_BRANCH=""
export TRAVIS_C=""
export TRAVIS_COMMIT=""
export TRAVIS_JOB_ID=""
export TRAVIS_JOB_ID=""
export TRAVIS_JOB_NUMBER=""
export TRAVIS_REPO_SLUG=""
export TRAVIS=""
export WERCKER_GIT_BRANCH=""
export WERCKER_GIT_COMMIT=""
export WERCKER_GIT_OWNER=""
export WERCKER_GIT_REPOSITORY=""
export WERCKER_MAIN_PIPELINE_STARTED=""
export WORKSPACE=""
export SYSTEM_TEAMFOUNDATIONSERVERURI=""
export BUILD_SOURCEVERSION=""
export BUILD_BUILDNUMBER=""
export PULL_REQUEST_NUMBER=""
export SYSTEM_PULLREQUEST_PULLREQUESTNUMBER=""
export SYSTEM_PULLREQUEST_PULLREQUESTID=""
export BUILD_BUILDID=""
export BUILD_SOURCEBRANCHNAME=""
export SYSTEM_TEAMPROJECT=""
export PULL_REQUEST_ID=""
export BITBUCKET_BRANCH=""
export BITBUCKET_BUILD_NUMBER=""
export BITBUCKET_REPO_OWNER=""
export BITBUCKET_REPO_SLUG=""
export BITBUCKET_PR_ID=""
export BITBUCKET_COMMIT=""
export GITHUB_ACTIONS=""
export GITHUB_HEAD_REF=""
export CODEBUILD_CI=""
export CODEBUILD_WEBHOOK_HEAD_REF=""
export CODEBUILD_RESOLVED_SOURCE_VERSION=""
export CODEBUILD_SOURCE_VERSION=""
export CODEBUILD_BUILD_ID=""
export CODEBUILD_SOURCE_REPO_URL=""
export HEROKU_TEST_RUN_BRANCH=""
export HEROKU_TEST_RUN_ID=""
export HEROKU_TEST_RUN_COMMIT_VERSION=""
export BUDDY=""
export BUDDY_EXECUTION_BRANCH=""
export BUDDY_EXECUTION_REVISION=""
export BUDDY_EXECUTION_ID=""
export BUDDY_EXECUTION_URL=""
export BUDDY_EXECUTION_PULL_REQUEST_NO=""
export BUDDY_EXECUTION_TAG=""
export BUDDY_REPO_SLUG=""
sleep 1
echo "SUCCESSFUL"
